...
**Status:** **All four v1.8 phases SHIPPED on 2026-08-24.** Phase 0 = commit `88a20e4`; Phase 1 = commit `7b33f1d`; Phase 2 = commit `2f90b50`; Phase 3 = commit `dcf38cf`. All four are pushed to `origin/main`. v1.8 (decision 0025) is COMPLETE. v1.9.x followups are tracked in `TASKS.md` sec 4 + `docs/decisions/v1.8-phase3-shipped.md` Followups (drag-drop queue reorder, per-provider usage caps, full Playwright e2e suite, MCP-on-Windows cleanup via decision 0026, FE-5..FE-9 wire-up). **Phase 3 was COMPLETED on 2026-08-24** — see §13.4 acceptance gate + §15 detailed plan in `0025` + `docs/decisions/v1.8-phase3-plan.md`. User approved all 5 open questions (Q1=a Phase 0 first; Q2=a snapshot to disk; Q3=Yes defer drag-drop to v1.9.x; Q4=c Read both; Q5=a hardcoded defaults + override). See `docs/decisions/0025-web-ui-ux-review-and-roadmap.md` §13.1 + §13.2 + §13.3 + §13.4 for the per-phase acceptance gates + §14.7 + §14.9 + §14.10 + §15 for the per-phase ship reports + §15 for the Phase 3 detailed plan.
