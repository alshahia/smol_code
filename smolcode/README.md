...
>
> **Phase 3** (in progress) — Dashboard + a11y + power features per `0025` §6.5. See `../docs/decisions/v1.8-phase3-plan.md` + `0025` §15 for the detailed plan; §13.4 for the acceptance gate. PREWORK: Vitest + Testing Library + axe-core + Playwright infra. New followup: render `Run.subagent_history` list in Inspector (UI consumes the wire field Phase 2 put down). All Phase 0/1/2 schema additions are additive + optional on the wire (older servers omit them, SPA renders empty state). See `../docs/decisions/0025-web-ui-ux-review-and-roadmap.md` for the full plan.
