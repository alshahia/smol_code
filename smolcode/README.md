...
> **Status:** **v1.8 Phase 0 shipped — Decision 0025 (Web UI/UX review + roadmap):** sub-agent events (`subagent.started` / `subagent.ended`) now flow through the orchestrator's `do_<tier>_task` and `do_specialist` tools so the SPA renders a nested `<SubAgentBlock>` for every delegation (A1 — the outer agent no longer hides the sub-agent's transcript). Token + step aggregates are auto-incremented in `Run.publish` for every `step.action` event under `pending_lock` and surfaced in the Inspector as "Token usage" (input / output / total + step count). A 1Hz client-side countdown ticks down the wall-clock budget so the user sees time-to-timeout instead of a frozen "running" state (B9 — the inspector also now clears `activeRun` cleanly when a run is purged mid-session, fixing the 404 screen). `WorkspaceTree` refreshes immediately on `diff.proposed` instead of waiting for its 10s poll (B11). Quick wins: `RunHistory` gains a task-text filter + Today/Yesterday/Earlier grouping; `TierSwitcher` falls back from a stale `full_access` selection with a dismissable warning; `ApiKeyPanel` no longer leaks its `confirm-forget` state on blur; thought / code_action / observations truncation bumped 2000 → 8000 chars with a "Show full" `<details>` toggle. Schema additions only (`RunSummary.tokens`, `RunSummary.step_count`, `RunSummary.remaining_s`, `RunSummary.subagent`, `StreamEvent.type` union adds `subagent.started` / `subagent.ended`); old servers that omit these fields leave them `undefined` so the SPA renders the empty state. See `../docs/decisions/0025-web-ui-ux-review-and-roadmap.md` for the full plan; Phases 1–3 (sessions, projects, pause/queue, file mentions, cost dashboard) are pending user acceptance of Phase 0.
