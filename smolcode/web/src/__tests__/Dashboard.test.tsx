...
const SAMPLE: api.DashboardResponse = {
  runs_today: 3,
  tokens_today: { input: 1000, output: 500, total: 1500 },
  errors_today: 1,
  by_provider: {
    openai: { input: 700, output: 300, total: 1000 },
    anthropic: { input: 300, output: 200, total: 500 },
  },
...
describe('Dashboard', () => {
  ...
  it('shows empty state when no providers', async () => {
    vi.spyOn(api, 'getDashboard').mockResolvedValue({ ...SAMPLE, by_provider: {}, runs_today: 0, tokens_today: { input: 0, output: 0, total: 0 } })
    render(<Dashboard />)
